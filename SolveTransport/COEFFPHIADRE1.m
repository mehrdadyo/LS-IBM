function [CM,AP,AW,AE,AS,AN] = COEFFPHIADRE(DOMAIN,Flux,IBM_coeffP)
...
for j = 2:jmax      % Loop over all interior pressure points
  for i = 2:imax
    ...
    if(i == 2 && j == 2)                    % Bottom-left corner
        AE_I(ind)=ind;
        AE_J(ind)=ind+1;
        AE(ind)=-(De(i,j)-(1-alphae(i,j))*Fe(i,j)-D1_a(j));
             
        AN_I(ind)=ind;
        AN_J(ind)=ind+jump;
        AN(ind)=-(Dn(i,j)-(1-alphan(i,j))*Fn(i,j));
      
        AP_I(ind)=ind;
        AP_J(ind)=ind;
        S_p=Fw(i,j)+D2_a(j);
        AP(ind)=-(AE(ind)+AN(ind))+dF(i,j)+A0_p(i,j)+S_p;
    ...
    elseif(i == 2 && j == jmax)             % Top-left corner
        ...
        AS(ind-jump)=-(Ds(i,j)+alphas(i,j)*Fs(i,j));
      
        AP_I(ind)=ind;
        AP_J(ind)=ind;
        S_p=Fw(i,j)+D2_a(j);
        AP(ind)=-(AE(ind)+AS(ind-jump))+dF(i,j)+A0_p(i,j)+S_p;
    ...
    elseif(i == imax && j == 2)             % Bottom-right corner
        ...
        AW_I(ind-1)=ind;
        AW_J(ind-1)=ind-1;
        AW(ind-1)=-(Dw(i,j)+alphaw(i,j)*Fw(i,j));
        ...
        AP_I(ind)=ind;
        AP_J(ind)=ind;
        AP(ind)=-(AW(ind-1)+AN(ind))+dF(i,j)+A0_p(i,j)+S_p;
    ...
    elseif(i == imax && j == jmax)          % Top-right corner
        ...
        AP_I(ind)=ind;
        AP_J(ind)=ind;
        AP(ind)=-(AW(ind-1)+AS(ind-jump))+dF(i,j)+A0_p(i,j)+S_p;
    ...
    elseif(i == 2)      % Left edge
        ...
        AP_I(ind)=ind;
        AP_J(ind)=ind;
        AP(ind)=-(AE(ind)+AW(ind-1)+AN(ind)+AS(ind-jump))+A0_p(i,j)+...
            dF(i,j)+S_p;
    ...
    elseif(j == 2)                          % Bottom edge
      ...
      AE_I(ind)=ind;
      AE_J(ind)=ind+1;
      AE(ind)=-(De(i,j)-(1-alphae(i,j))*Fe(i,j));
      ...
      AP_I(ind)=ind;
      AP_J(ind)=ind;
      AP(ind)=-(AE(ind)+AW(ind-1)+AN(ind))+A0_p(i,j)+dF(i,j)+S_p;
    elseif(j == jmax)                       % Top edge
      ...
      AP_I(ind)=ind;
      AP_J(ind)=ind;
      AP(ind)=-(AE(ind)+AW(ind-1)+AS(ind-jump))+A0_p(i,j)+...
          dF(i,j)+S_p;
    else                                    % All others
        AE_I(ind)=ind;
        AE_J(ind)=ind+1;
